...
function postProcessing(images, imagesFiles, user, file, logger, traceId) {
  setTimeout(async () => {
    try {
      ...
      if (metadata && metadata.exif) {
        update = {
          ...
          width: metadata.exif.PixelXDimension,
          height: metadata.exif.PixelYDimension,
      ...
      if (metadata && metadata.image) {
        update = {
          ...
          exif: {
            ...pick(metadata.image, ["Make", "Model"]),
            ...update.exif,
          },
        };
      }
...
async function v1(config, logger) {
  ...
  return async (req, res) => {
    ...
    if (file.mimetype && file.mimetype.endsWith("jpeg")) {
      postProcessing(images, imagesFiles, user, file, logger, traceId);
    }
